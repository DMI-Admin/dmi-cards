...
export async function GET(request: Request, context: { params: Promise<{ assetId: string }> }) {
  ...
  try {
    ...
    const { data: asset, error } = await timing.measure("asset_lookup", () => database.from("card_media_assets").select("*").eq("id", assetId).maybeSingle());
    if (error || !asset || !["ready", "attached"].includes(asset.state)) throw unavailable();
    const { data: session } = await timing.measure("session_lookup", () => database.from("card_media_sessions").select("*").eq("id", asset.session_id).eq("owner_user_id", client.userId).maybeSingle());
    if (!session || (session.state === "pending" && Date.parse(session.expires_at) <= Date.now()) || session.state === "expired") throw unavailable();
    if (session.card_id) {
      const { data: card } = await timing.measure("card_lookup", () => database.from("cards").select("id").eq("id", session.card_id).eq("user_id", client.userId).maybeSingle());
