...
export async function GET(_request: Request, context: { params: Promise<{ slug: string; kind: string }> }) {
  try {
    ...
    const { data: card } = await database.from("cards").select("*").eq("id", cardId).maybeSingle();
    if (!card || !(card.status === "published" || card.is_published === true)) throw unavailable();
    const { data: rawTemplate } = await database.from("templates").select("*").eq("id", card.template_id).or("status.eq.published,is_published.eq.true").maybeSingle();
    ...
    const { data: asset } = await database.from("card_media_assets").select("*").eq("id", assetId).eq("kind", kind).eq("state", "attached").maybeSingle();
    if (!asset) throw unavailable();
    const { data: session } = await database.from("card_media_sessions").select("*").eq("id", asset.session_id).eq("owner_user_id", card.user_id).eq("card_id", cardId).eq("state", "attached").maybeSingle();
