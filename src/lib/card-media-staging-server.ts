...
export async function beginCardMediaSession(request: Request, templateId: string, cardId?: string) {
  const { client, database } = await authorizedTemplate(request, templateId);
  if (cardId) {
...
export async function prepareCardMediaAsset(request: Request, sessionId: string, kind: CardMediaKind, bytes: Buffer) {
  ...
  } catch { return invalid("Image could not be decoded safely."); }
